...
export default function CreateNewCourse({ params }: PageProps) {
    ...
    // Extract series ID from dynamic route
    const seriesId = resolvedParams.id && resolvedParams.id.length > 0 && resolvedParams.id[0] !== 'new' ? resolvedParams.id[0] : null
    const isNewCourse = !seriesId || (resolvedParams.id && resolvedParams.id[0] === 'new')
    
    // Debug logging
    console.log('Route params:', resolvedParams.id, 'SeriesId:', seriesId, 'IsNewCourse:', isNewCourse)
    ...
    // Fetch data only if editing existing course
