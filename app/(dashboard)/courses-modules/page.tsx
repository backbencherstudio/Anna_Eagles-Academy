...
export default function CoursesModulesPage() {
  ...
  const handleNextVideo = useCallback(() => {
    ...
    if (videoIndex < currentModule.videos.length - 1) {
  ...
  return (
    <div className={`flex gap-6 max-w-7xl mx-auto py-6 transition-all duration-500 ease-in-out ${
      isTheaterMode ? 'flex-col max-w-full px-0' : 'flex-col lg:flex-row px-4'
    }`}>
      {/* Video Player Section */}
      <div className={`bg-white rounded-2xl shadow transition-all duration-500 ease-in-out ${
        isTheaterMode ? 'w-full p-0' : 'flex-1 p-6'
      }`}>
        {/* Video Player */}
        <CustomVideoPlayer
        ...
        {!isTheaterMode && (
          <div className="flex items-start justify-between mb-4">
            ...
            <div className="flex gap-2">
              ...
              >
                {isFirstVideo() ? 'First Video' : 'Previous'}
              </button>
              <button
              ...
              >
                {isLastVideo() ? 'Last Video' : 'Next Video'}
              </button>
            </div>
          </div>
        )}
      ...
      <div className={`bg-white rounded-2xl shadow transition-all duration-500 ease-in-out ${
        isTheaterMode ? 'w-full p-6' : 'w-full lg:w-80 p-6'
      }`}>
                    ...
                    {mod.videos.map((vid: any, vidIdx: number) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center cursor-pointer gap-3 px-3 py-2 rounded-lg text-left transition font-medium text-sm w-full ${
                            isActive
