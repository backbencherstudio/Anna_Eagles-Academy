...
const completeCourseData: CompleteCourseData = {
    ...
    courseTitle: "Course-2: The Kingdom of God is all about Character",
    ...
    lessons: [
        {
            id: "1",
            title: "Lesson 1: Kingdom in Daily Life",
        ...
        {
            id: "3",
            title: "Lesson 3: Kingdom in Daily Life",
            duration: "25min 32sec",
            isCompleted: false,
            isActive: false,
            isLocked: true
            ...
            <Accordion type="single" collapsible className="w-full" defaultValue="course-content">
                <AccordionItem value="course-content" className="border-none">
                    ...
                    <AccordionContent className="pt-4 sm:pt-6 space-y-4 sm:space-y-6">
                        ...
                        <div className='border border-[#ECEFF3] rounded-xl p-4'>
                            ...
                            <div className="space-y-2 sm:space-y-3 mt-4">
                                ...
                                {completeCourseData.lessons.map((lesson, index) => (
                                    <Card
                                        key={lesson.id}
                                        className={`border transition-all duration-200 hover:shadow-md ${lesson.isActive
                                            ? 'bg-[#FFFBF6] border-[#F1C27D] cursor-pointer shadow-sm' // 
                                            : lesson.isLocked
                                    ...
                                    >
                                        <CardContent className="p-3 sm:p-4">
                                            <div className="flex items-center gap-3">
                                                <div className={`w-6 h-6 sm:w-10 sm:h-10 rounded-full flex items-center justify-center transition-all duration-200 flex-shrink-0 ${lesson.isActive
                                                    ? 'bg-[#F1C27D]'
                                                    : lesson.isLocked
                                                        ? 'bg-gray-400'
                                                        : lesson.isCompleted
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <h3 className={`font-medium transition-colors text-sm sm:text-base leading-tight ${lesson.isActive
                                                        ? 'text-black'
                                                        : lesson.isLocked
                                                            ? 'text-gray-500'
                                                            : lesson.isCompleted
                                                    ...
                                                    <p className={`text-xs sm:text-sm mt-1 ${lesson.isLocked ? 'text-gray-400' : 'text-gray-600'
                                                        }`}>
                                                        {lesson.duration}
                                                    </p>
                                                </div>
                        ...
                        {/* Recap Video Card */}
                        <Card className="border border-[#ECEFF3] opacity-50">
                            <CardContent className="p-3 sm:p-4">
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
                                    <div className="flex items-center gap-3">
                                    ...
                                    <Button variant="outline" size="sm" className="flex items-center gap-2 bg-blue-50 border-blue-200 text-[#0F2598] hover:bg-[#ECEFF3] cursor-not-allowed text-xs sm:text-sm px-3 py-2 w-full sm:w-auto justify-center" disabled>
                                        <Play className="w-3 h-3 sm:w-4 sm:h-4" />
